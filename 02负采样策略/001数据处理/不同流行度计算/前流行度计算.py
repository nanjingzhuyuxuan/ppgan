...
config = {
    '100k':{
        ...
        'train_pop_txt':'../../../01数据集/100k/front_train_pop.csv',
    ...
    'ml1m': {
        ...
        'train_pop_txt': '../../../01数据集/ml1m/front_train_pop.csv',
    ...
    'cd_movies': {
        ...
        'train_pop_txt': '../../../01数据集/cd_movies/front_train_pop.csv',
...
class Pop:
    ...
    def get_record_pop(self):
        ...
        for index,item in self.train.iterrows():
            ...
            data = self.train[self.train.time <= item_time ]
            len_data = len(data)
            pop = len(data[data['iid']==iid])/len_data
            self.train.loc[index,'pop_value'] = round(np.sqrt(np.sqrt(pop)),5)
