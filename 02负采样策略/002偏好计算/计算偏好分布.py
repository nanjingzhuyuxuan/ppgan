...
class Pop:
    ...
    def get_record_pop(self):
        ...
        data_high = []
        for i in range(0,10):
            u_p = user_pop_sort[i][1]
            u = user_pop_sort[i][0]
            u_i_w = data_interest[u]
            for x,j in u_i_w.items():
                if j>1000:
                    continue
                data_high.append([u_p, self.item_pop[x], j])

        df = pd.DataFrame(data_high, columns=['u', 'i','w'])
        plt.scatter(df.u, df.i, color='#023047', alpha=0.3, s=df.w*200)
        plt.xlabel('u_pop')
        plt.ylabel('i_pop')
